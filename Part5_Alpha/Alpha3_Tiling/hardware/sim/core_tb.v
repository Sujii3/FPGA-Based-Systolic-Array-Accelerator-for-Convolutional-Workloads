...
// edits
wire [44:0] inst_q; // 16 * 2 instruction size + edits
...
wire [col*psum_bw-1:0] core_out2;

integer x_file, x_scan_file ; // file_handler
integer w_file, w_scan_file ; // file_handler
integer acc_file, acc_scan_file ; // file_handler
integer acc_file2, acc_scan_file2 ; // file_handler
...
initial begin 
  ...
  // start of first 8 output channels tb
  part2 = 1;
  x_file = $fopen("../datafiles/alpha3_files/activation.txt", "r");
  ...
  for (kij=0; kij<9; kij=kij+1) begin  // kij loop
    case(kij)
      0: w_file = $fopen("../datafiles/alpha3_files/weight_k1.txt", "r");
      1: w_file = $fopen("../datafiles/alpha3_files/weight_k2.txt", "r");
      2: w_file = $fopen("../datafiles/alpha3_files/weight_k3.txt", "r");
      3: w_file = $fopen("../datafiles/alpha3_files/weight_k4.txt", "r");
      4: w_file = $fopen("../datafiles/alpha3_files/weight_k5.txt", "r");
      5: w_file = $fopen("../datafiles/alpha3_files/weight_k6.txt", "r");
      6: w_file = $fopen("../datafiles/alpha3_files/weight_k7.txt", "r");
      7: w_file = $fopen("../datafiles/alpha3_files/weight_k8.txt", "r");
      8: w_file = $fopen("../datafiles/alpha3_files/weight_k9.txt", "r");
    endcase
    // Following three lines are to remove the first three comment lines of the file
  ...
  // finish of first 8 output channels tb

  // start of last 8 output channels tb
  part2 = 1;
  x_file = $fopen("../datafiles/alpha3_files/activation.txt", "r");
  // Following three lines are to remove the first three comment lines of the file
  x_scan_file = $fscanf(x_file,"%s", captured_data);
  ...
  for (kij=0; kij<9; kij=kij+1) begin  // kij loop
    case(kij)
      ...
      8: w_file = $fopen("../datafiles/alpha3_files/weight_k9.txt", "r");
    endcase
    // Following 16 loops are to remove the first 16 weights from the file + 3 heading lines
    ...
    for (t=0; t < 2*col; t=t+1) begin  
      #0.5 clk = 1'b0;  w_scan_file = $fscanf(w_file,"%32b", D_xmem); WEN_xmem = 0; CEN_xmem = 0; if (t>0) A_xmem = A_xmem + 1; 
      #0.5 clk = 1'b1;  
      //if (kij == 0) $display("weight read: %32b", D_xmem);
    end
  ...
  ////////// Accumulation /////////
  out_file = $fopen("../datafiles/alpha3_files/out.txt", "r");  
  acc_file = $fopen("../datafiles/acc_scan.txt", "r");
  acc_file2 = $fopen("../datafiles/acc_scan2.txt", "r");
  ...
  $display("############ Verification Start during accumulation of Alpha3 #############"); 
  ...
  for (i=0; i<len_onij+1; i=i+1) begin 
    ...
    if (i>0) begin
     out_scan_file = $fscanf(out_file,"%256b", answer); // reading from out file to answer
       if ({core_out2, core_out} == answer)
         $display("%2d-th output featuremap Data matched! :D", i); 
       else begin
         $display("%2d-th output featuremap Data ERROR!!", i); 
         $display("core_out: %128b", core_out);
         $display("core_out2: %128b", core_out2);
    ...
    for (j=0; j<len_kij+1; j=j+1) begin 
      ...
      #0.5 clk = 1'b0;   
        ...
        if (j<len_kij) begin CEN_pmem = 0; WEN_pmem = 1; acc_scan_file2 = $fscanf(acc_file2,"%11b", A_pmem2); end
                       else  begin CEN_pmem = 1; WEN_pmem = 1; end
